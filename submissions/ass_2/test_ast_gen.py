...
def test_exp_21():
    source = "void main() { a = b(); }"
    expected = "Program([FuncDecl(VoidType(), main, [], [ExprStmt(AssignExpr(Identifier(a) = FuncCall(Identifier(b), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_exp_30():
    ...
    expected = "Program([FuncDecl(VoidType(), fun, [], []), FuncDecl(VoidType(), main, [], [ExprStmt(AssignExpr(Identifier(a) = BinaryOp(FuncCall(Identifier(f), []), *, BinaryOp(IntLiteral(3), *, IntLiteral(3)))))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_exp_31():
    source = "void fun() {} void main() { a = (f() + 3) * 3; }"
    expected = "Program([FuncDecl(VoidType(), fun, [], []), FuncDecl(VoidType(), main, [], [ExprStmt(AssignExpr(Identifier(a) = BinaryOp(BinaryOp(FuncCall(Identifier(f), []), +, IntLiteral(3)), *, IntLiteral(3))))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_func_5():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [ExprStmt(FuncCall(Identifier(fun), [IntLiteral(2), IntLiteral(5), FuncCall(Identifier(fun), []), BinaryOp(IntLiteral(3), +, IntLiteral(5)), MemberAccess(Identifier(fun).mandy), BinaryOp(MemberAccess(Identifier(fun).mandy), +, BinaryOp(IntLiteral(4), *, IntLiteral(21)))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_func_6():
    source = "void main() { fun(clock, ++3, 3++, f++, --g, g--, smearing_bunny); }"
    expected = "Program([FuncDecl(VoidType(), main, [], [ExprStmt(FuncCall(Identifier(fun), [Identifier(clock), PrefixOp(++IntLiteral(3)), PostfixOp(IntLiteral(3)++), PostfixOp(Identifier(f)++), PrefixOp(--Identifier(g)), PostfixOp(Identifier(g)--), Identifier(smearing_bunny)]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_func_7():
    source = "struct hammond {int a; float b; string c; }; int lmao() {} void fun() {} void main() { fun(); }"
    expected = "Program([StructDecl(hammond, [MemberDecl(IntType(), a), MemberDecl(FloatType(), b), MemberDecl(StringType(), c)]), FuncDecl(IntType(), lmao, [], []), FuncDecl(VoidType(), fun, [], []), FuncDecl(VoidType(), main, [], [ExprStmt(FuncCall(Identifier(fun), []))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_func_9():
    ...
    expected = "Program([StructDecl(hammond, [MemberDecl(IntType(), a), MemberDecl(FloatType(), b), MemberDecl(StringType(), c)]), FuncDecl(IntType(), lmao, [], []), FuncDecl(VoidType(), fun, [], []), FuncDecl(VoidType(), main, [], [ExprStmt(FuncCall(Identifier(fun), []))]), FuncDecl(FloatType(), lmao2, [], []), FuncDecl(StringType(), lmao3, [], [])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_struct_4():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [VarDecl(IntType(), a = MemberAccess(FuncCall(Identifier(abc), []).d))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_if_1():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if Identifier(pong_tin) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_2():
    source = "void main() {if(pong_tin) lmao();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if Identifier(pong_tin) then ExprStmt(FuncCall(Identifier(lmao), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_3():
    source = "void main() {if(pong_tin) { lmao(); } else { porsche(); } }"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if Identifier(pong_tin) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), []))]), else BlockStmt([ExprStmt(FuncCall(Identifier(porsche), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_4():
    source = "void main() {if(pong_tin) lmao(); else stone_henge();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if Identifier(pong_tin) then ExprStmt(FuncCall(Identifier(lmao), [])), else ExprStmt(FuncCall(Identifier(stone_henge), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_5():
    source = "void main() {if(pong_tin) { lmao(); } else spot_of_lunch(); }"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if Identifier(pong_tin) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), []))]), else ExprStmt(FuncCall(Identifier(spot_of_lunch), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_6():
    source = "void main() {if(pong_tin && on_paper ) { lmao(); } else spot_of_lunch(); }"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if BinaryOp(Identifier(pong_tin), &&, Identifier(on_paper)) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), []))]), else ExprStmt(FuncCall(Identifier(spot_of_lunch), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_7():
    source = "void main() {if(pong_tin && on_paper || (pretending * wahoo)) { lmao(); } else spot_of_lunch(); }"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if BinaryOp(BinaryOp(Identifier(pong_tin), &&, Identifier(on_paper)), ||, BinaryOp(Identifier(pretending), *, Identifier(wahoo))) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), []))]), else ExprStmt(FuncCall(Identifier(spot_of_lunch), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_8():
    source = "void main() {if(ahk || pong_tin && on_paper + 35 || (pretending * wahoo())) { lmao(); } else spot_of_lunch(); }"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if BinaryOp(BinaryOp(Identifier(ahk), ||, BinaryOp(Identifier(pong_tin), &&, BinaryOp(Identifier(on_paper), +, IntLiteral(35)))), ||, BinaryOp(Identifier(pretending), *, FuncCall(Identifier(wahoo), []))) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), []))]), else ExprStmt(FuncCall(Identifier(spot_of_lunch), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_if_9():
    source = "void main() {if(pong_tin) { lmao(); a = 3 + 7; int s = 35;} else if(lmao) {lmao(); lmao();} else peaked();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [IfStmt(if Identifier(pong_tin) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), [])), ExprStmt(AssignExpr(Identifier(a) = BinaryOp(IntLiteral(3), +, IntLiteral(7)))), VarDecl(IntType(), s = IntLiteral(35))]), else IfStmt(if Identifier(lmao) then BlockStmt([ExprStmt(FuncCall(Identifier(lmao), [])), ExprStmt(FuncCall(Identifier(lmao), []))]), else ExprStmt(FuncCall(Identifier(peaked), []))))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_block_statement_2():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [BlockStmt([VarDecl(IntType(), a), VarDecl(IntType(), b = FuncCall(Identifier(fun), [])), VarDecl(IntType(), c = BinaryOp(BinaryOp(BinaryOp(Identifier(b), *, Identifier(a)), +, IntLiteral(3)), -, FuncCall(Identifier(fun), [])))])])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_while_1():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [WhileStmt(while Identifier(hammond) do BlockStmt([ExprStmt(FuncCall(Identifier(boat), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_while_2():
    source = "void main() {while(hammond) boat();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [WhileStmt(while Identifier(hammond) do ExprStmt(FuncCall(Identifier(boat), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_while_3():
    source = "void main() {while(hammond) { boat(); a = b + c; box(); }}"
    expected = "Program([FuncDecl(VoidType(), main, [], [WhileStmt(while Identifier(hammond) do BlockStmt([ExprStmt(FuncCall(Identifier(boat), [])), ExprStmt(AssignExpr(Identifier(a) = BinaryOp(Identifier(b), +, Identifier(c)))), ExprStmt(FuncCall(Identifier(box), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_while_4():
    source = "void main() {while(hammond || propeller + c) { boat(); a = b + c; box(); }}"
    expected = "Program([FuncDecl(VoidType(), main, [], [WhileStmt(while BinaryOp(Identifier(hammond), ||, BinaryOp(Identifier(propeller), +, Identifier(c))) do BlockStmt([ExprStmt(FuncCall(Identifier(boat), [])), ExprStmt(AssignExpr(Identifier(a) = BinaryOp(Identifier(b), +, Identifier(c)))), ExprStmt(FuncCall(Identifier(box), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_while_5():
    source = "void main() {while(8 * hammond || propeller + c) { boat(); a = b + c; box(); }}"
    expected = "Program([FuncDecl(VoidType(), main, [], [WhileStmt(while BinaryOp(BinaryOp(IntLiteral(8), *, Identifier(hammond)), ||, BinaryOp(Identifier(propeller), +, Identifier(c))) do BlockStmt([ExprStmt(FuncCall(Identifier(boat), [])), ExprStmt(AssignExpr(Identifier(a) = BinaryOp(Identifier(b), +, Identifier(c)))), ExprStmt(FuncCall(Identifier(box), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_1():
    source = "void main() {for(int i = 0; i < 10; ++i) {korean_crane_carrier();}}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); BinaryOp(Identifier(i), <, IntLiteral(10)); PrefixOp(++Identifier(i)) do BlockStmt([ExprStmt(FuncCall(Identifier(korean_crane_carrier), []))]))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_2():
    source = "void main() {for(int i = 0; i < 10; ++i) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); BinaryOp(Identifier(i), <, IntLiteral(10)); PrefixOp(++Identifier(i)) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_3():
    source = "void main() {for(int i = 0; i < 10; ) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); BinaryOp(Identifier(i), <, IntLiteral(10)); None do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_4():
    source = "void main() {for(int i = 0; ; ++i) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); None; PrefixOp(++Identifier(i)) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_5():
    source = "void main() {for(int i = 0; ; ) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); None; None do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_6():
    source = "void main() {for(; i < 10; ++i) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for None; BinaryOp(Identifier(i), <, IntLiteral(10)); PrefixOp(++Identifier(i)) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_7():
    source = "void main() {for(; i < 10; ) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for None; BinaryOp(Identifier(i), <, IntLiteral(10)); None do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_8():
    source = "void main() {for(; ; ++i) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for None; None; PrefixOp(++Identifier(i)) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_9():
    source = "void main() {for(int i = 0; i < 10; i++) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); BinaryOp(Identifier(i), <, IntLiteral(10)); PostfixOp(Identifier(i)++) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_for_11():
    source = "void main() {for(int i = 0; ; i++) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); None; PostfixOp(Identifier(i)++) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_for_13():
    source = "void main() {for(; i < 10; i++) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for None; BinaryOp(Identifier(i), <, IntLiteral(10)); PostfixOp(Identifier(i)++) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_for_15():
    source = "void main() {for(; ; i++) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for None; None; PostfixOp(Identifier(i)++) do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected

def test_for_16():
    source = "void main() {for(; ; ) korean_crane_carrier();}"
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for None; None; None do ExprStmt(FuncCall(Identifier(korean_crane_carrier), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_for_33():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [ForStmt(for VarDecl(IntType(), i = IntLiteral(0)); BinaryOp(Identifier(i), <, IntLiteral(10)); PrefixOp(++Identifier(i)) do ExprStmt(FuncCall(Identifier(pete), [])))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_exp_39():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [ExprStmt(AssignExpr(Identifier(a) = PrefixOp(!PrefixOp(--FuncCall(Identifier(foo), [])))))])])"
    assert str(ASTGenerator(source).generate()) == expected


def test_exp_40():
    source = "void main() { a = bar.baz().x + 1; }"
    expected = "Program([FuncDecl(VoidType(), main, [], [ExprStmt(AssignExpr(Identifier(a) = BinaryOp(MemberAccess(FuncCall(MemberAccess(Identifier(bar).baz), []).x), +, IntLiteral(1))))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_integration_3():
    ...
    expected = "Program([FuncDecl(VoidType(), f, [], []), FuncDecl(IntType(), g, [Param(IntType(), x)], [ReturnStmt(return BinaryOp(Identifier(x), *, IntLiteral(2)))]), FuncDecl(VoidType(), main, [], [VarDecl(IntType(), y = FuncCall(Identifier(g), [FuncCall(Identifier(f), []), IntLiteral(1)]))])])"
    assert str(ASTGenerator(source).generate()) == expected
...
def test_stress_3():
    ...
    expected = "Program([FuncDecl(VoidType(), main, [], [ExprStmt(AssignExpr(Identifier(result) = BinaryOp(BinaryOp(MemberAccess(Identifier(a).b), +, FuncCall(Identifier(foo), [Identifier(c), MemberAccess(Identifier(d).e)])), *, PrefixOp(-BinaryOp(Identifier(x), -, Identifier(y))))))])])"
    assert str(ASTGenerator(source).generate()) == expected
