...
class StaticChecker(ASTVisitor):
    ...
    def check_struct_redeclared(self, name, scope):
        struct_namespace = self.get_struct_namespace(scope)

    def check_func_redeclared(self, name, scope):
        func_namespace = self.get_func_namespace(scope)

        if(any([True for struct in func_namespace if name == struct[0].name])):
            raise Redecalred("Struct", name)

    def check_redeclared(self, kind, name, local_scope):
        if(any([True for nm in local_scope if name == nm])):
            raise Redeclared(kind, name)

    def check_undeclared(self, kind, name, global_scope):
        if(not any([True for scope in global_scope for nm in scope if name == nm])):
    ...
    def visit_binary_op(self, node: "BinaryOp", o: Any = None):
        left_expr = self.visit(node.left, o)
        right_expr = self.visit(node.right, o)

        return o

    def visit_prefix_op(self, node: "PrefixOp", o: Any = None):
        operand_expr = self.visit(node.operand, o)
    ...
    def visit_assign_expr(self, node: "AssignExpr", o: Any = None):
        lhs_expr = self.visit(node.lhs, o)
        rhs_expr = self.visit(node.rhs, o)
